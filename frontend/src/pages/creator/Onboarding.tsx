...
const CreatorOnboarding: React.FC = () => {
  ...
  const fetchUserEmailAndPhone = async () => {
    try {
      const res = await fetch(config.SERVER_URL + '/creator/search-creator-cred', {
        method: 'POST',
        headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch(config.SERVER_URL + '/creator/onboarding', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${localStorage.getItem('token')}`
    ...
    <div className="flex min-h-screen mb-5">
      ...
      <div className="w-1/3 flex items-center justify-center">
        <div className="text-left">
          <h1 className="text-2xl font-bold mb-4">Onboarding as a Creator</h1>
          <p className="text-gray-700">
            Make yourself available for hire by providing accurate and detailed
            information. This will help clients connect with you effectively.
          </p>
        </div>
      </div>
